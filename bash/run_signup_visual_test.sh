...
TEST_FOLDER="../tests/visual/signup_visual"

# корневая папка для отчетов
REPORT_ROOT="../reports/signup_visual"
...
/home/yegor/.local/bin/pytest "$TEST_FOLDER" --html="$REPORT_FOLDER/${REPORT_FILE}.tmp" --self-contained-html -v
mv "$REPORT_FOLDER/${REPORT_FILE}.tmp" "$REPORT_FOLDER/$REPORT_FILE"

echo
echo "Отчет сформирован: $REPORT_FOLDER/$REPORT_FILE"
echo "Доступно по: https://uc4.ru/frontend-autotests-reports/signup_visual/${DATE_STR}/${REPORT_FILE}"